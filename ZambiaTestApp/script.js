...
window.addEventListener('beforeinstallprompt', e => {
  e.preventDefault();      // 기본 브라우저 설치 알림 막기
  deferred = e;            // 이벤트 저장
  document.getElementById('installBtn').hidden = false; // 버튼 보이기
});

document.getElementById('installBtn').addEventListener('click', async () => {
  if (!deferred) return;
  deferred.prompt();           // 설치 다이얼로그 띄우기
  await deferred.userChoice;   // 사용자가 선택할 때까지 대기
  deferred = null;             // 이벤트 초기화
  document.getElementById('installBtn').hidden = true; // 버튼 다시 숨기기
});
